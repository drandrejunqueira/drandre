...
.cinematic-footer-wrapper {
  ...
  /* Dynamic Variables using project's custom tokens */
  --pill-bg-1: color-mix(in oklch, var(--color-text) 3%, transparent);
  --pill-bg-2: color-mix(in oklch, var(--color-text) 1%, transparent);
  --pill-shadow: color-mix(in oklch, var(--color-off-white) 50%, transparent);
  --pill-highlight: color-mix(in oklch, var(--color-text) 10%, transparent);
  --pill-inset-shadow: color-mix(in oklch, var(--color-off-white) 80%, transparent);
  ...
  --pill-shadow-hover: color-mix(in oklch, var(--color-off-white) 70%, transparent);
...
.footer-bg-grid {
  ...
  background-image: 
    linear-gradient(to right, color-mix(in oklch, var(--color-text) 3%, transparent) 1px, transparent 1px),
    linear-gradient(to bottom, color-mix(in oklch, var(--color-text) 3%, transparent) 1px, transparent 1px);
...
.footer-aurora {
  background: radial-gradient(
    circle at 50% 50%, 
    color-mix(in oklch, var(--color-green) 15%, transparent) 0%, 
...
.footer-giant-bg-text {
  ...
  color: transparent;
  -webkit-text-stroke: 1px color-mix(in oklch, var(--color-text) 5%, transparent);
  background: linear-gradient(180deg, color-mix(in oklch, var(--color-text) 10%, transparent) 0%, transparent 60%);
...
/* Metallic Text Glow */
.footer-text-glow {
  background: linear-gradient(180deg, var(--color-text) 0%, color-mix(in oklch, var(--color-text) 40%, transparent) 100%);
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
  background-clip: text;
  filter: drop-shadow(0px 0px 20px color-mix(in oklch, var(--color-text) 15%, transparent));
}
`;
...
const MarqueeItem = () => (
  <div className="flex items-center space-x-12 px-6">
    <span>Excelência Ortopédica</span> <span className="text-primary/60">✦</span>
    <span>Medicina Regenerativa</span> <span className="text-secondary/60">✦</span>
    <span>Cuidado Humanizado</span> <span className="text-primary/60">✦</span>
    <span>Prevenção & Performance</span> <span className="text-secondary/60">✦</span>
    <span>Inovação Médica</span> <span className="text-primary/60">✦</span>
  </div>
);
        ...
        <footer className="fixed bottom-0 left-0 flex h-screen w-full flex-col justify-between overflow-hidden bg-background text-foreground cinematic-footer-wrapper">
          ...
          <div className="relative z-10 flex flex-1 flex-col items-center justify-center px-6 mt-20 w-full max-w-5xl mx-auto">
            ...
            <div ref={linksRef} className="flex flex-col items-center gap-6 w-full">
              ...
              <div className="flex flex-wrap justify-center gap-4 w-full">
                <MagneticButton as="a" href="https://wa.me/5512999999999" className="footer-glass-pill px-10 py-5 rounded-full text-foreground font-bold text-sm md:text-base flex items-center gap-3 group">
                ...
                <MagneticButton as="a" href="#contact" className="footer-glass-pill px-10 py-5 rounded-full text-foreground font-bold text-sm md:text-base flex items-center gap-3 group">
              ...
              <div className="flex flex-wrap justify-center gap-3 md:gap-6 w-full mt-2">
                <MagneticButton as="a" href="#specialties" className="footer-glass-pill px-6 py-3 rounded-full text-muted-foreground font-medium text-xs md:text-sm hover:text-foreground">
                  Especialidades
                </MagneticButton>
          ...
          <div className="relative z-20 w-full pb-8 px-6 md:px-12 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="text-muted-foreground text-[10px] md:text-xs font-semibold tracking-widest uppercase order-2 md:order-1">
              © 2026 Dr. André Elias Junqueira. Todos os direitos reservados.
            </div>
